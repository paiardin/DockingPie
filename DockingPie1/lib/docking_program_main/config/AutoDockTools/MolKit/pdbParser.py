...
from .molecule import Atom, Bond, HydrogenBond, AtomSet
from .moleculeParser import MoleculeParser
from .protein import Protein, Chain, Residue, ProteinSet
...
"""
        ...
        if callable(parser):
            ...
            if key == 'ATOM':
                parser = self.parse_PDB_atoms
            elif key == 'HETATM':
                parser = self.parse_PDB_atoms
            elif key == 'CRYST1':
                parser = self.parse_PDB_CRYST1
            elif key == 'CONECT':
                parser = self.parse_PDB_CONECT
            elif key == 'SSBOND':
                parser = self.parse_PDB_SSBOND
            elif key == 'HELIX':
                parser = self.parse_PDB_Helix
            elif key == 'TURN':
                parser = self.parse_PDB_Turn
            elif key == 'SHEET':
                parser = self.parse_PDB_Strand
            elif key == 'HYDBND':
                parser = self.parse_PDB_HYDBND
            elif key == 'MODRES':
                parser = self.parse_PDB_MODRES
            else:
                raise ValueError('No default parser for %s record, You have to provide one as the second argument' %
                                 key)
            self.pdbRecordParser[key] = parser
        ...
        for line in lines:
            ...
            if originalName in AAnames:  # ti is a modified amino acid
                ...
                if originalName in DNAnames:
                    ...
                    print('WARNING: MODRES %s not recognized in %s' % (originalName, line))
    ...
    def parse(self, objClass=Protein):
        ...
        if 'ATOM' in self.pdbRecordParser:
            ...
            ##                     beg = t+1
                ...
                if self.modelsAs == 'molecules':  # create a new molecule for each model
                    ...
                    raise ValueError(
                        "bad value modelsAs, expected 'molecules', 'chains' or 'conformations', got" + self.modelsAs)
        ...
        for record in self.pdbRecordParser:
            ...
            # self.updateProgressBar()
            if record == 'HETATM':
                continue  # they get parsed with ATOM
            elif record == 'ATOM':
                continue
            else:
    ...
    def parse_PDB_ATOM_record(self, rec, mol):
        ...
        if resSeq != mol.curRes.number:
            ...
            if res:
                ...
                mol.curRes = Residue(type=resName, number=resSeq, parent=mol.curChain, top=mol)
                # Add a hasCA and hasO flags to the residue and set it to 0
                mol.curRes.hasCA = 0
                mol.curRes.hasO = 0
...
class PdbqParser(PdbParser):
    ...
    def parse_PDB_REMARK(self, rec):
        ...
        found = 0
        for item in rec:
            if item.find('active torsions') > -1:
                l = item
                found = 1
                llist = l.split()
        ...
        if rec[0].find("ADVS_result") > -1:  # second version
            ...
            for ll in rec:
                if ll.find('hb_atoms') > -1:
                    ...
                    if in_hb:
                        hb.append(ll[10:])  # skip 'USER','AD>' AND '\n'
                    elif in_macro_close_ats:
                        macro_close_ats.append(ll[10:-1])
                    elif in_lig_close_ats:
                        lig_close_ats.append(ll[10:-1])
                    elif in_pi_cation:
                        pi_cation.append(ll[10:-1])
                    elif in_pi_pi:
                        pi_pi.append(ll[10:-1])
                    elif in_cluster:
                        self.cl_lines.append(ll[10:])
    ...
    def parse_PDB_REMARK(self, rec):
        ...
        for item in rec:
            if item.find('active torsions') > -1:
                ...
                llist = l.split()
                self.mol.ndihe = int(llist[1])
            if item.find('VINA RESULT') > -1:
                # print "processing vr=", item
                l = item
                llist = l.split()
                vina_results.append(llist[3:])
            elif item.find('AD4 RESULT') > -1:
                # print "processing vr=", item
                l = item
                llist = l.split()
