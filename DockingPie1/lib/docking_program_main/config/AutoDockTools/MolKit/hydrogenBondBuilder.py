...
def getAngle(ac, hat, don):
    ...
    hCoords = hat.coords
    dCoords = don.coords
    pt1 = np.array(acCoords, 'f')
    pt2 = np.array(hCoords, 'f')
    pt3 = np.array(dCoords, 'f')
    # pt1 = np.array(ac.coords, 'f')
    # pt2 = np.array(hat.coords, 'f')
    # pt3 = np.array(don.coords, 'f')
    v1 = np.array(pt1 - pt2)
    v2 = np.array(pt3 - pt2)
    dist1 = math.sqrt(np.sum(v1 * v1))
    dist2 = math.sqrt(np.sum(v2 * v2))
    sca = np.dot(v1, v2) / (dist1 * dist2)
    if sca > 1.0:
        sca = 1.0
    elif sca < -1.0:
        sca = -1.0
...
class HydrogenBondBuilder:
    ...
    def buildD(self, ats, paramDict=None):
        ...
        hAts = AtomSet(hydrogen_atoms.get(lambda x, donorTypes=donorTypes:
                                          x.bonds[0].atom1.babel_type in donorTypes or x.bonds[0].atom2.babel_type in
                                          donorTypes))
        d['hAts'] = hAts
    ...
    def process(self, dict1, dict2, paramDict):
        ...
        if distOnly:
            ...
            self.makeBonds(atDict, donor2Ats, donor3Ats, acceptor2Ats, acceptor3Ats, paramDict)
            return atDict

        badAtDict = self.filterBasedOnAngs(atDict, donor2Ats, donor3Ats, acceptor2Ats, acceptor3Ats, paramDict)
    ...
    def getDonors(self, nodes, paramDict):
        ...
        if sp2DAts:
            if hats:
                hsp2 = AtomSet(hats.get(lambda x, sp2DAts=sp2DAts: x.bonds[0].atom1 in sp2DAts or x.bonds[0].atom2 in
                                                                   sp2DAts))
        if sp2DAts:
        ...
        if sp3DAts:
            if hats:
                hsp3 = AtomSet(
                    hats.get(lambda x, sp3DAts=sp3DAts: x.bonds[0].atom1 in sp3DAts or x.bonds[0].atom2 in sp3DAts))
        hsp = hsp2 + hsp3
        # print 'hsp=', hsp.name
