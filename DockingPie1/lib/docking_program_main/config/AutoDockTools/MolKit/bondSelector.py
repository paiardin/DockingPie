...
from ..MolKit.molecule import BondSet, AtomSet
    ...
    def select(self, bnds, cutoff=7.5):
        ...
        for ring in rf.rings:
            ...
            for bnd in blist:
                ...
                pt2 = at2.coords
                pt3 = at3.coords
                a1 = np.subtract(pt2, pt1)
                b1 = np.subtract(pt3, pt2)
                p = [a1[1] * b1[2] - a1[2] * b1[1], a1[2] * b1[0] - a1[0] * b1[2], a1[0] * b1[1] - a1[1] * b1[0]]
                result0 = np.sqrt(p[0] * p[0] + p[1] * p[1] + p[2] * p[2])
                bnd.nrmsize = result0
                result1 = p
                bnd.nrms = result1
                # next find the other bond w/atom2:
                z2 = list(filter(lambda x, bnd=bnd, at2=bnd.atom2, blist=blist: x != bnd and x.atom1 == at2 or
                                                                                x.atom2 == at2, blist))
...
class PeptideAromaticCycleBondSelector(BondSelector):
    ...
    """
    bondD = aromDict = {}
    bondD['TRP'] = ['NE1', 'CD1', 'CG', 'CD2', 'CE2', 'CZ2', 'CH2', 'CZ3', 'CE3', 'AD1', 'AG', 'AD2', 'AE2', 'AZ2',
                    'AH2', 'AZ3', 'AE3']
    bondD['TYR'] = ['CD1', 'CG', 'CD2', 'CE1', 'CE2', 'CZ', 'AD1', 'AG', 'AD2', 'AE1', 'AE2', 'AZ']
    bondD['PHE'] = ['CD1', 'CG', 'CD2', 'CE1', 'CE2', 'CZ', 'AD1', 'AG', 'AD2', 'AE1', 'AE2', 'AZ']
    bondD['HIS'] = ['CD2', 'CE1', 'CG', 'ND1', 'NE2', 'AD2', 'AE1', 'AG']
