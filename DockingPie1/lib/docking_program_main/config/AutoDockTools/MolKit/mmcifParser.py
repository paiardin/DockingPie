...
from .molecule import Atom, AtomSet, HydrogenBond
    ...
    def processHelData(self, chain):
        ...
        for index in range(number_of_records):
            if beg_asym_id[index] == chain.id and conf_id[index].find('HELX') == 0:
                startData = beg_comp_id[index] + beg_PDB_ins_code[index].strip('?') + beg_seq_id[index]
                startData = startData.strip()
                startRes = chain.residues.get(startData)

                if len(startRes) != 1:
                    print(("ERROR: When parsing the HELIX information found %d %s in chain %s" % (len(startRes),
                                                                                                  startData, chain.id)))
                    continue
                endData = end_comp_id[index] + end_PDB_ins_code[index].strip('?') + end_seq_id[index]
                endData = endData.strip()
                endRes = chain.residues.get(endData)

                if len(endRes) != 1:
                    print(("ERROR: When parsing the HELIX information found %d %s in chain %s" % (len(endRes),
                                                                                                  endData, chain.id)))
                    continue
    ...
    def processStrData(self, chain):
        ...
        for index in range(number_of_records):
            if beg_asym_id[index] == chain.id:
                ...
                if len(startRes) != 1:
                    print(("ERROR: When parsing the SHEET information found %d %s in chain %s" % (len(startRes),
                                                                                                  startData, chain.id)))
                    continue
                ...
                if len(endRes) != 1:
                    print(("ERROR: When parsing the SHEET information found %d %s in chain %s" % (len(endRes),
                                                                                                  endData, chain.id)))
                    continue
                ...
                if type(order_sheet_id) == list:
                    for tmp_index in range(len(order_sense)):
                        if sheet_range_id[index] == order_sheet_id[tmp_index]:
                            if (order_range_id_1[tmp_index] == range_id[index] or order_range_id_2[tmp_index] ==
                                    range_id[index]):
                ...
                strStartEndData.append({'start': startRes[0], 'end': endRes[0], 'nbStrand': nbStrand, 'sense': sense})
    ...
    def processTurnData(self, chain):
        ...
        for index in range(number_of_records):
            if beg_asym_id[index] == chain.id and conf_id[index].find('TURN') == 0:
                ...
                startRes = chain.residues.get(startData)
                if len(startRes) != 1:
                    print(("ERROR: When parsing the TURN information found %d %s in chain %s" % (len(startRes),
                                                                                                 startData, chain.id)))
                    continue
                endData = end_comp_id[index] + end_PDB_ins_code[index].strip('?') + end_seq_id[index]
                endData = endData.strip()
                endRes = chain.residues.get(endData)
                if len(endRes) != 1:
                    print(("ERROR: When parsing the TURN information found %d %s in chain %s" % (len(endRes),
                                                                                                 endData, chain.id)))
                    continue
