...
from .molecule import Atom, AtomSet, BondSet
...
class InteractionDescriptor:
    ...
    def buildHydrogenBonds(self):
        ...
        if len(macro_hb_ats):
            macro_hbas = d['macro_hbas'] = AtomSet(
                macro_hb_sidechains + macro_hb_gly_atoms + macro_hb_ats)  # all from macro
    ...
    def getCations(self, atoms):
        ...
        arg_cations = atoms.get(lambda x: (x.parent.type == 'ARG' and x.name in ['CZ']))
        lys_cations = atoms.get(lambda x: (x.parent.type == 'LYS' and x.name in ['NZ', 'HZ1', 'HZ2', 'HZ3']))
        # select any positively-charged metal ions... cannot include CA here
        metal_cations = atoms.get(lambda x: x.name in ['Mn', 'MN', 'Mg', 'MG', 'FE', 'Fe', 'Zn', 'ZN'])
    ...
    def detectPiInteractions(self, tolerance=0.95, debug=False, use_all_cycles=False):
        ...
        if debug:
            print("LIG: len(l_rf.rings)=", len(l_rf.rings))
        if not len(l_rf.rings):
            if debug:
                print("no lig rings found by l_rf!")
            return
        ...
        if debug:
            print("LIG: len(lig_arom_rings)=", len(lig_rings))
        ...
        if len(lig_rings):
            ...
            if len(macro_cations):
                if debug:
                    print("check distances from lig_rings to macro_cations here")
        ...
        # rf.rings is list of dictionaries, one per ring, with keys 'bonds' and 'atoms'
        if debug:
            print("MACRO: len(m_rf.rings)=", len(m_rf.rings))
        if not len(m_rf.rings):
            if debug:
                print("no macro rings found by m_rf!")
            return
        macro_rings = []
        for r in m_rf.rings:
            ring_bnds = r['bonds']
        ...
        if debug:
            print("len(macro_arom_rings)=", len(macro_rings))
        ...
        if len(macro_rings):
            ...
            if len(lig_cations):
                if debug:
                    print("check distances from macro_rings to lig_cations here")
        ...
        for lig_ring_bnds in lig_rings:
            ...
            lig_atoms.sort()
            if debug:
                print("len(lig_atoms)=", len(lig_atoms))
            # ---------------------------------
            # compute the normal to lig ring
            # ---------------------------------
            a1 = np.array(lig_atoms[0].coords)
            a2 = np.array(lig_atoms[2].coords)
            a3 = np.array(lig_atoms[4].coords)
            if debug:
                print("a1,a2, a3=", a1.tolist(), a2.tolist(), a3.tolist())
            for macro_ring_bnds in macro_rings:
                macro_atoms = acbs.getAtoms(macro_ring_bnds)
                macro_atoms.sort()
                if debug:
                    print("len(macro_atoms)=", len(macro_atoms))
                ...
                # ---------------------------------
                b1 = np.array(macro_atoms[0].coords)
                b2 = np.array(macro_atoms[2].coords)
                b3 = np.array(macro_atoms[4].coords)
                if debug:
                    print("b1,b2, b3=", b1.tolist(), b2.tolist(), b3.tolist())
                # check for stacking
                a2_1 = a2 - a1
                a3_1 = a3 - a1
                b2_1 = b2 - b1
                ...
                if debug:
                    print("n1_dot_n2", np.dot(n1, n2))
                if abs(n1_dot_n2) >= 1 * tolerance:
                    if debug:
                        print("The rings are stacked vertically")
                ...
                if abs(n1_dot_n2) <= 0.01 * tolerance:
                    if debug:
                        print("The rings are stacked perpendicularly")
                    new_result = (acbs.getAtoms(lig_ring_bnds), acbs.getAtoms(macro_ring_bnds))
