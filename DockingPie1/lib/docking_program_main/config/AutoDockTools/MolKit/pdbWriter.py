...
class PdbWriter(MoleculeWriter):
    ...
    def write(self, filename, nodes, sort=False, sortFunc=None,
        ...
        if isinstance(parser, PdbParser):
            fileRec = [x for x in records if x not in self.FROMDATASTRUCT]
            for rec in fileRec:
    ...
    def addRecord(self, key, record=[]):
        ...
        for line in record:
            ...
            if constraints == 1:
                i = 0
                for value in line:
                    if PDBFormatConstr[key][i] is None:
                        pass
                    else:
    ...
    def defineHYDBNDRecords(self, atoms):
        ...
        for a in atoms:
            ...
            for b in a.hbonds:
                ...
                if b.donAt.top != b.accAt.top:
                    continue
                if b.hAt is not None and b.donAt.top != b.hAt.top:
                    continue
    ...
    def defineSecondaryStructureSection(self, mol, origin='File'):
        ...
        if origin == 'File':
            ...
            parser = mol.parser
            if isinstance(parser, PdbParser):
                if not parser.hasSsDataInFile():
                    return
                self.recordsToWrite['HELIX'] = parser.getRecords(parser.allLines, 'HELIX')
                self.recordsToWrite['SHEET'] = parser.getRecords(parser.allLines, 'SHEET')
                self.recordsToWrite['TURN'] = parser.getRecords(parser.allLines, 'TURN')
            ...
            # parsed
                ...
                for chain in mol.chains:
                    ...
                    if helices is not None:
                        allhelices = allhelices + helices
                    strands = sSet.get(lambda x: x.name.startswith('Strand'))
                    if strands is not None:
                        allstrands = allstrands + strands
        ...
        # 2- Create the SS record from the information generated by stride
            ...
            if mol.hasSS not in ['From Stride', "From PROSS"]:
                return
            else:
                allstrands = SecondaryStructureSet([])
                allhelices = SecondaryStructureSet([])
...
class PdbqWriter(PdbWriter):
    ...
    def write_records(self, file, molecule):
        ...
        for key in tags:
            if key in self.userRecords and len(self.userRecords[key]) != 0:
                RecordList = self.userRecords[key]
            elif (isPdbParser and len(Parser.getRecords(Parser.allLines, '%s' % key)) != 0
                  and ((key in mandatory) or (key in required) or key in self.userRecords)):
        ...
        for line in RecordList:
            ...
            elif key in ['ORIGX1', 'ORIGX2', 'ORIGX3', 'SCALE1', 'SCALE2', 'SCALE3', 'MTRIX1', 'MTRIX2', 'MTRIX3']:
