...
class ConformationHandler:
                ...
                if entry == 'bonds':
                    if mag_diff > cutoff:
                        ...
                        if verbose: print(confInd, ": Distance %d%s-%d%s differs: % 6.4f %6.4f \n" \
                                          % (k[0] - 1, self.mol.allAtoms[k[0] - 1].name, k[1] - 1,
                                             self.mol.allAtoms[k[1] - 1].name, refv, v))
                else:
                    if mag_diff > angle_cutoff:
                        found_different_angle = True
                        if verbose: print(confInd, ": Angle %d%s-%d%s-%d%s differs: % 6.4f %6.4f \n" \
                                          % (k[0] - 1, self.mol.allAtoms[k[0] - 1].name, k[1] - 1,
                                             self.mol.allAtoms[k[1] - 1].name, k[2] - 1,
                                             self.mol.allAtoms[k[2] - 1].name, refv, v))
    ...
    def set_conformation(self, conf, index=None):
        ...
        if hasattr(conf, 'ad4_estat_energies') and conf.ad4_estat_energies and len(conf.ad4_estat_energies) == len(
                allAtoms):
            for n in range(len(allAtoms)):
