...
class MSMS(msms.MOLSRF):
    ...
    def compute(self, probe_radius=1.5, density=1.0, hdensity=3.0,
                allComponents=0, nbSeedAtoms=0, seedAtoms=None,
                restartMode='full'):
        """err <- self.compute(probe_radius=1.5, density=1.0,
                    allComponents=False, nbSeedAtoms=0, seedAtoms=None,
                    restartMode='full')
    ...
    allComponent Boolean, When True cavities are searched for and surfaces
                 This option makes the program substantially slower
    
    nbSeedAtoms can be 0, 1 2 or 3 and indicates the number of atoms indices
    in atoms used to seed the probe sphere for the calculation
    restartMode -- full or partial. Full destroys the entire reduced surface
                   and restarts. Slower but more robust
    """
        if restartMode=='full':
            # destroy the entire reduced surface and restart
            # slower but more robust
            self.restart_mode = 0
        else:
            # destroy the reduced surface around atoms with changed radii
            # faster but less robust
            self.restart_mode = 1
            
        # set seed atoms for reduced surface calculation
        if nbSeedAtoms>0: # we want a specific component only
            assert nbSeedAtoms in (1,2,3)
            self.rsr.fnba = nbSeedAtoms
            self.rsr.set_ffat( -1, -1, -1 )
            seed = [-1, -1, -1]
            for i in range(nbSeedAtoms):
                assert seedAtoms[i]>0 and seedAtoms[i]<self.nbat
                seed[i] = seedAtoms[i]
            self.rsr.set_ffat( seed[0], seed[1], seed[2] )
            return msms.MS_compute_surface(self.this, probe_radius, density,
                                           hdensity)

        elif allComponents: # we want all outer components and cavities
            self.all_components = allComponents
            return msms.MS_compute_surface(self.this, probe_radius, density,
                                           hdensity)

        else: # we want all outer components
            # indices of atom with extreme coords

            # compute the first component
            ok = msms.MS_compute_surface(self.this, probe_radius, density,
                                          hdensity)
            if ok == msms.MS_ERR:
                return msms.MS_ERR
            extremes = self._extreme().tolist() # xmin xmax ymin ymax, zmin zmax

            # min and max values for atom centers along 3 axis
            minia = numpy.min( self.coords, 0)
            maxia = numpy.max( self.coords, 0)

            allVf= []
            compNum = 0
            while True:
                # compute bounding box of surface
                vf, dum, dum = self.getTriangles(component=compNum)
                _vf = vf[:,:3]
                allVf.extend(_vf.tolist())
                minis = numpy.min(allVf, 0)
                maxis = numpy.max(allVf, 0)
                if (minia > minis).all() and (maxia < maxis).all():
                    break # all atoms inside surface BB
                else:
                    # find seed atom (outside surface BB)
                    for index in extremes:
                        c = self.coords[index]
                        if (c > minis).all() and (c < maxis).all():
                            # this atom is inside the surface BB
                            extremes.remove(index)
                        else: #this is our new seed
                            seed = index
                            break
                    ok = msms.MS_add_component(self.this, seed)
                    if ok==msms.MS_ERR:
                        print 'Failed to add component seeded at atom %d\n'%seed
                        return msms.MS_ERR
                    compNum += 1
            return msms.MS_OK
...
"""
    ...
    """
        ...
        faces = numpy.take(tri, faceInd, axis=0)
...
##  	if len(find):
##  	    fac = numpy.take( self.sesf, find, axis=0)
...
##  	    vts = numpy.take( self.sesv, vind, axis=0 )
##  	return vts, fac
...
##  	    fac = numpy.take( self.sesf[:,:3], ind , axis=0)
##  	    mat = numpy.take( self.sesfColors, ind , axis=0)
##  	elif faces == 'reent':
...
##  	    assert len(faces) > 0
##  	    if analytical == 0 :
##  		fac = numpy.take( self.sesf[:,:3], list(faces), axis=0 )
##  		mat = numpy.take( self.sesfColors, list(faces), axis=0 )
##  	    else:
...
##  	triCenters = numpy.take( self.sesv[:,:3], self.sesf[:,:3], axis=0 )
...
##  	else:
##  	    assert len(triang) > 0
##  	    labs = numpy.take( atlabs, list(triang), axis=0 )
##  	    pos = numpy.take( triCenters, list(triang), axis=0 )
...
##  	    ind = numpy.nonzero(fact)
##  	    if len(ind):
##  		fac = numpy.take( triangs, ind, axis=0 )
##  		facCenter = numpy.take( verts, fac , axis=0)
...
##  	    if analytical == 0 :
##  		pos = numpy.take( self.sesv[:,:3], list(vertices), axis=0 )
##  	    else:
...
##  		    vertsFlag[f[2]] = 1;
##  		vind = numpy.nonzero(vertsFlag)
##  		pos = numpy.take( self.sesv[:,:3], vind, axis=0 )
##  		labs = map(str, vind)
