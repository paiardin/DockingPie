...
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/DlgFilters.py,v 1.6.6.3 2016/02/23 18:44:51 annao Exp $
#
# $Id: DlgFilters.py,v 1.6.6.3 2016/02/23 18:44:51 annao Exp $
#
#
#
#
...
class InteractionFilter(Filter):       
    ...
    def __init__(self, receptor_file):
        ...
        #len(rec_coords)
        #1844
        checkRadii = numpy.array(rec_vdw_rad, 'f')
        self.bigRC = numpy.resize(checkRadii, (lenC, lenC)) #receptor info
        self.bigC = numpy.resize(rec_coords, (lenC,lenC,3))
        ###bigR = bigRC[:lenK]
    ...
    def setup_ligand(self, lig_lines):
        ...
        self.smallM = numpy.array(lig_coords, 'f')
        #setup variables for ligand arrays
        if first_lig:
            self.lenK=len(lig_coords)
            self.keyRadii = numpy.array(self.lig_vdw_rad, 'f')
    ...
    def filter(self, dlg):
        ...
        for model_lines in all_models:
            ...
            ansMat = numpy.logical_and(numpy.less(dSQMAT, cutoffSQMAT),numpy.not_equal(dSQMAT, 0.))
