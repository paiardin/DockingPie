    ...
    is_receptor = False):
        ...
        for obj in cmd.get_names("objects") + cmd.get_names("selections"):
            if re.search("Run_", str(obj)):
                ...
                self.first_tmp_list.append(str(obj))

        #self.first_tmp_list = [str(obj) for obj in cmd.get_names("objects") + cmd.get_names("selections") if not re.search("Run", obj)]

        if self.is_receptor:

            # First list is needed to check for objects with multiple states.
            # If multiple states are present, the user is asked to choose wether to split or not them
            for pymol_obj in self.first_tmp_list:
                type = PyMOL_get_type(self, pymol_obj)
                #type = cmd.get_type(pymol_obj)

                if type.type == str("object:molecule"):
                    atom_number = PyMOL_count_atoms(self, pymol_obj, state = 1)

                # if type.type == str("object:molecule") and atom_number.atom_number >= 99:
                # try:
                #     cmd.count_atoms(pymol_obj, state = 1)
                #     if type == str("object:molecule") and cmd.count_atoms(pymol_obj, state = 1) >= 99:
                #         self.check_for_multiple_states(pymol_obj)
                #         if self.has_multiple_state:
                #             self.split_states_dialog(pymol_obj)
                #
                # except Exception as e:
                #     if type == str("object:molecule"):
                #         print(str("PyMOL v 2.5 bug. Trying to fix, the coordinates of " + str(pymol_obj) + " are loaded again"))
                #         cmd.delete(pymol_obj)
                #         path_to_obj = os.path.join(self.tab.parent().docking_programs.tmp_dir_path, str(pymol_obj + ".pdb"))
                #         cmd.load(path_to_obj, pymol_obj)
                #
                #     if type == str("object:molecule") and cmd.count_atoms(pymol_obj, state = 1) >= 99:
                #         self.check_for_multiple_states(pymol_obj)
                #         if self.has_multiple_state:
            ...
            self.tmp_list = [str(obj) for obj in cmd.get_names("objects") + cmd.get_names("selections") if not re.search("Run", obj)]
            self.scrolledlist_items = []
            # Add to the importable object list, only the Receptor from all the objects (self.tmp_list) loaded in PyMOL
            for i in self.tmp_list:

                type = PyMOL_get_type(self, i)
                #type = cmd.get_type(pymol_obj)

                if type.type == str("object:molecule"):
                    atom_number = PyMOL_count_atoms(self, i, state = 1)
                ...
                if type.type == str("object:molecule") and atom_number.atom_number >= 99:
                    ...
                    if self.has_multiple_state:
                        cmd.split_states(i)
                        cmd.delete(i)
                    self.scrolledlist_items.append(str(i))

                # try:
                #     type = cmd.get_type(i)
                #     if type == str("object:molecule") and cmd.count_atoms(i, state = 1) >= 99:
                #         self.scrolledlist_items.append(str(i))
                #
                # except Exception as e:
                #         print("PyMOL v 2.5 bug")
